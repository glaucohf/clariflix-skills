...
## EMAIL SWIPE ACTIVATION LAYER

Before drafting the sequence, load the local swipe assets below whenever the sequence touches appointment setting, objection handling, reactivation, no-show recovery, or last-mile conversion.

- `swipe/email/hammer-them-examples.md`
  Use for appointment-conditioning cadence, expectation-setting, objection sequencing, price transparency timing, and proof-led follow-up.
- `swipe/tactics/break-the-glass-tactics.md`
  Use for re-engagement, stalled leads, no-response lists, and low-friction revival messages.

Use these files for:

- sequence architecture benchmarking
- angle extraction
- cadence selection
- recovery-message simplification

Do NOT use them for:

- copy-paste reuse
- fake urgency
- forcing aggressive recovery tactics into warm nurture sequences

Minimum benchmark pass:

1. Pick 2-4 patterns that match the sequence type.
2. Name the pattern being borrowed.
3. Rewrite it in the offer's message architecture and voice.
4. Note where the benchmark influenced the final sequence map.
...
optional_inputs:
  ...
  swipe_benchmarks:
    description: "Local benchmark files used for email architecture and recovery sequencing"
    default:
      - "swipe/email/hammer-them-examples.md"
      - "swipe/tactics/break-the-glass-tactics.md"
```
