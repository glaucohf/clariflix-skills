...
Observação: `sync-ide-command.py`, `validate-squad-structure.py`, `squad-analytics.py` e `yaml_validator.py` hoje são wrappers de compatibilidade para workers canônicos do `squad-creator`, enquanto `refresh-registry.py` permanece um worker Pro-native ligado ao registry do pack.
